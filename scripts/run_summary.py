...
def _process_dataframe_summary_sync(
    df: pd.DataFrame,
    vllm_client: OpenAI,
    model_name: str,
    prompts_config: dict,
    ...
    # Load taxonomy for context definitions (same structure as in run_processing.py)
    ...
    config = {
        ...
        "processing": {
            ...
            # Classification tokens are unused here but kept for interface parity.
            "max_tokens_classification": 0,
        },
    ...
    with tqdm(
        ...
        for row in df_to_process.itertuples():
            ...
            if log_progress:
                logger.info(f"Summarizing row {row.Index}")
    ...
    logger.info(f"Summarization complete. Processed {row_counter} rows.")
    return df_processed


async def _summarize_row_async(row, processor: AsyncMessyTextProcessor, semaphore):
    """
...
async def _process_dataframe_summary_async(
    df: pd.DataFrame,
    ...
    logger.info(f"Starting async summarization with {max_concurrent_rows} concurrent rows.")

    tasks = []
    for row in df_to_process.itertuples():
        tasks.append(_summarize_row_async(row, processor, semaphore))

    results_list: list[dict] = []
    for coro in tqdm_async.as_completed(
        tasks,
        total=len(tasks),
        desc="Summarizing Rows",
        disable=not use_progress_bar,
    ):
        result = await coro
        results_list.append(result)
        if log_resources and len(results_list) % 100 == 0:
            log_memory_usage(logger, f"Summarized {len(results_list)} rows")

    if results_list:
        results_df = pd.DataFrame(results_list).set_index("index")
        df_processed.update(results_df)

    logger.info(f"Summarization complete. Processed {len(results_list)} rows.")
    return df_processed
...
def main() -> None:
    ...
    # Load prompt configuration (same key as in run_processing.py)
    prompts_path = settings["paths"].get("prompts", "config/prompts.json")
    with open(prompts_path, "r", encoding="utf-8") as f:
        prompts_config = json.load(f)

    # Load data from summary input path
    ...
    logger.info("=" * 50)
    logger.info("PRE-FLIGHT CHECKS (Summary-only)")
    logger.info("=" * 50)
    ...
    if async_processing:
        logger.info("Using ASYNC summarization mode.")
        ...
        processed_df = asyncio.run(
            _process_dataframe_summary_async(
                df=df_text,
        ...
        logger.info("Using SYNC summarization mode.")
        processed_df = _process_dataframe_summary_sync(
            df=df_text,
