...
**This paper is a work in progress and is being shared for review in its current incomplete form. This research is supported by NSF funding. The argument, analysis, results and references are still under revision, and this draft should not be treated as a final version of the paper.**
i
# Abstract

When working with real-world text, researchers often inherit corpora and annotations that preserve costly human judgment but were not collected, organized, or annotated for modern text-as-data workflows. This problem is especially common in social science domains where web-scraped documents are noisy, incomplete, or misaligned with the original unit of analysis. We discovered that all you need is codebook attention. This problem is solved with a codebook-guided turn-by-turn extractive summarization pipeline that reuses valuable annotations without requiring full reannotation. For each victim in a corpus of disappearance reports from Mexico, the model processes documents sequentially, extracts verbatim evidence spans for each codebook category, and updates a running structured summary to produce a clean and evidence-traceable document-level representation. Using the codebook as the task schema focuses model attention on the fields that matter for downstream classification and reconnects prior human annotations to contemporary language model workflows. We evaluate the resulting summaries by classifying them and comparing the predicted labels with human annotations across multiple open-source models. Because the pipeline records turn-by-turn outputs, it also identifies where errors enter the workflow and documents the marginal gains and costs of additional turns and annotations, making the human and machine trade explicit. Preliminary results suggest that hours of computation can approximate weeks of human coding effort while producing evidence-traceable summaries suitable for downstream human rights and event data research.
...
- HTTP error pages / missing pages: target URL was unreachable
- Ancillary or obscured text on pages: website with extraneous information such as navigation menus, footers, and advertising banners interleaved with the article body, obscuring the relevant text.
- Irrelevant content: content that is topically adjacent but are substantially irrelevant, such as suggested article previews or user comment threads appended to the original report.

Each of these error types introduces noise that can mislead or degrade downstream text processing. Each error type has consequences for downstream tasks such as fine-tuning based on a noisy corpus. When a model is fine-tuned on such documents, the training signal is distributed across the full input including the noise, making it difficult for the model to learn which spans are responsible for a given label. Distractive content such as suggested articles or comment threads poses a more subtle but more dangerous risk, as the text is topically connected to disappearance reporting and can cause the model to attribute information from unrelated cases to the victim under analysis, resulting in hallucinated or incorrectly sourced output.
...
The task is to transform a set of messy, multi document news reports associated with a single victim into a structured, evidence traceable document level representation that supports reuse of the existing codebook and annotations.
...
Evaluation of summarization tasks compares the text summary with a gold standard reference text. Given the messy and noisy initial documents, it is difficult to use them as reference texts.  The prior work has human annotations classifying the information in the summary text. The evaluation tests whether generated summaries reproduce the same labels as the human annotations, comparing a simple summarization baseline against the proposed method, across major model families, and across model sizes.
...
The current results indicate that the proposed conversational extractive workflow improves performance over the simple generative baseline on a substantial share of the evaluated labels, with the strongest gains appearing in categories that benefit from cross document accumulation and evidence tracking. The clearest improvements appear in perpetrator and legal procedure fields. For the field on who carried out the threats, accuracy rises from 0.44 to 0.79, precision rises from 0.07 to 0.21, and recall rises from 0.11 to 0.28. For the field on the type of sentence against perpetrators or detained individuals, accuracy rises from 0.76 to 0.95, and the field on the type of tribunal or court improves from 0.56 to 0.76. The method also improves the field on which authority responded to the contact from 0.16 to 0.36 in accuracy, the field on the type of place from which the victim disappeared from 0.25 to 0.49, the field on treatment during captivity from 0.64 to 0.80, and the field on whether the victim belonged to a distinct social group from 0.39 to 0.49. These gains are consistent with the claim that a multi turn stateful workflow is especially useful when relevant information is distributed across several reports and must be preserved by item rather than condensed into a single freeform summary.

At the same time, the gains are not uniform across all labels. Some categories show weaker performance or declines under the current setup, including the field on the involvement of civil society in the case, where accuracy falls from 0.40 to 0.26, the field on the method of capture and the language used to describe the disappearance, where accuracy falls from 0.42 to 0.27, and the field on the outcome of the disappearance, where accuracy falls from 0.54 to 0.39. Other fields remain only modestly improved in recall even when accuracy rises. Taken together, these preliminary results suggest that the proposed method is promising but label dependent. It appears to work best for fields that require aggregating scattered evidence across documents, while categories with higher ambiguity or weaker textual cues remain harder to recover reliably. These findings are still preliminary, and broader cross model comparisons and additional evaluation remain ongoing.
...
Analytics for Noisy Unstructured Text Data I. 2009. *Encyclopedia of Artificial Intelligence*. https://doi.org/10.4018/978-1-59904-849-9.CH015.

Agarwal, Sumeet, Shantanu Godbole, Diwakar Punjani, and Shourya Roy. 2007. "How Much Noise Is Too Much: A Study in Automatic Text Classification." In *Proceedings of the 7th IEEE International Conference on Data Mining*, 3-12. https://doi.org/10.1109/ICDM.2007.21.

Contractor, Danish, Tanveer A. Faruquie, and L. Venkata Subramaniam. 2010. "Unsupervised Cleansing of Noisy Text." In *Proceedings of COLING 2010: Posters*, 189-196. https://aclanthology.org/C10-2022/.

Hariharakrishnan, Jayaram, S. Mohanavalli, Srividya, and K. B. S. Kumar. 2017. "Survey of Pre-processing Techniques for Mining Big Data." *2017 International Conference on Computer, Communication and Signal Processing*. https://doi.org/10.1109/ICCCSP.2017.7944072.

Johnson, Justin M., and T. Khoshgoftaar. 2022. "A Survey on Classifying Big Data with Label Noise." *ACM Journal of Data and Information Quality*. https://doi.org/10.1145/3492546.

Lee, Ga-young, Lubna Alzamil, Bakhtiyar Doskenov, and Arash Termehchy. 2021. "A Survey on Data Cleaning Methods for Improved Machine Learning Model Performance." *arXiv.org*. https://arxiv.org/abs/2109.07127.

Roy, Shourya, and L. Subramaniam. 2009. "Analytics for Noisy Unstructured Text Data I." *Encyclopedia of Artificial Intelligence*. https://doi.org/10.4018/978-1-59904-849-9.CH015.

Stuhler, Oscar, Cat Dang Ton, and Etienne Ollion. 2025. "From Codebooks to Promptbooks: Extracting Information from Text with Generative Large Language Models." *Sociological Methods & Research* 54(3): 794-848. https://doi.org/10.1177/00491241251336794.

Subramaniam, L. V., Shourya Roy, T. Faruquie, and Sumit Negi. 2009. "A Survey of Types of Text Noise and Techniques to Handle Noisy Text." *Workshop on Analytics for Noisy Unstructured Text Data*. https://doi.org/10.1145/1568296.1568315.

Subramaniam, L. V., and Shourya Roy. 2009. "Analytics for Noisy Unstructured Text A Data II." *Encyclopedia of Artificial Intelligence*. https://doi.org/10.4018/9781599048499.ch016.

Than, Nga, Leanne Fan, Tina Law, Laura K. Nelson, and Leslie McCall. 2025. "Updating “The Future of Coding”: Qualitative Coding with Generative Large Language Models." *Sociological Methods & Research* 54(3): 849-888. https://doi.org/10.1177/00491241251339188.

Xiong, Hui, G. Pandey, M. Steinbach, and Vipin Kumar. 2006. "Enhancing Data Analysis with Noise Removal." *IEEE Transactions on Knowledge and Data Engineering*. https://doi.org/10.1109/TKDE.2006.46.

Zhou, Wei, Jun Zhou, Haoyu Wang, Zhenghao Li, Qikang He, Shaokun Han, Guoliang Li, Xuanhe Zhou, Yeye He, Chunwei Liu, Zirui Tang, Bin Wang, Shen Tang, Kai Zuo, Yuyu Luo, Zhenzhe Zheng, Conghui He, Jingren Zhou, and Fan Wu. 2026. "Can LLMs Clean Up Your Mess? A Survey of Application-Ready Data Preparation with LLMs." *arXiv* 2601.17058. https://arxiv.org/abs/2601.17058.
