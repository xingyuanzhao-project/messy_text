...
format:
  revealjs:
    ...
    slide-number: true
    incremental: false
    footer: "This research was funded by NSF (OAC-2311142)."
---
...
- 575 victims, 2,229 news reports across five states.
- Coded by trained human coders against a 7-section codebook.
...
## Why the dataset is messy for downstream tasks

1. **Error pages**: the URL was unreachable.
2. **Page components**: navigation, menus, ads, footers interleaved with body text.
3. **Topical distractors**: suggested articles, comment threads about *other* cases.
4. **Unit-of-analysis misalignment**: the annotations are made at the victim level.
...
1. **Extract** verbatim spans for each codebook field from each document.
...
Output is clean, label-structured, and traceable back to source spans.
...
## Why human annotations as the gold standard
...
## Simple summarization vs Extractive summarization

::: {style="transform: scale(1.4); transform-origin: top center; margin-bottom: 8rem;"}
::: {layout-ncol=3}
![Outcome (desenlace)](plots/evaluation/benchmarks/less_labels_multi_models/metrics_desenlace.png){width="100%"}

![Place of capture](plots/evaluation/benchmarks/less_labels_multi_models/metrics_captura_tipo.png){width="100%"}

![Social group](plots/evaluation/benchmarks/less_labels_multi_models/metrics_vic_grupo_social.png){width="100%"}
:::
:::
...
| Model | Time | GPU |
|---|---:|---|
| Llama-3.1-8B-Instruct-AWQ-INT4 | ~5 h 27 min | A100 |
...
## Portability to other legacy corpora
...
| Model | Time (full corpus) | GPU |
|---|---:|---|
| Llama 3.1 8B Instruct (AWQ INT4) | ~5 h 27 min | A100 |
| Ministral 3 8B Instruct | ~51 min | A100 |
| Gemma 3 12B IT (AWQ INT4) | ~38 min | A100 |
